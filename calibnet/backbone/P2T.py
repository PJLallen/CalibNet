...
def build_p2t_backbone(cfg, input_shape):
    name = cfg.MODEL.P2T.NAME
    out_features = cfg.MODEL.P2T.OUT_FEATURES
    in_channels = input_shape.channels

    assert name == "base" or name == "large"
    if name == "base":
        return PyramidPoolingTransformer(
        patch_size=4,
...
class PyramidPoolingTransformer(Backbone):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,attn_drop_rate=0., drop_path_rate=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 9, 3], out_features=None): #
        super().__init__()
        print("loading p2t")
    ...
    def forward_features(self, x):
        ...
        if self.out_features_names[0] in self._out_features:
            outs[self.out_features_names[0]] = x
        ...
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        if self.out_features_names[1] in self._out_features:
            outs[self.out_features_names[1]] = x
        ...
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        if self.out_features_names[2] in self._out_features:
            outs[self.out_features_names[2]] = x
        ...
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        if self.out_features_names[3] in self._out_features:
            outs[self.out_features_names[3]] = x
